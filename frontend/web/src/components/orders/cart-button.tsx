...
export function FloatingCartBar() {
  ...
  if (!hasItems || shouldHideBottomBar) {
    return null;
  }
  ...
  return (
    <Link
      href="/cart"
      className="fixed inset-x-4 bottom-24 z-40 flex min-h-16 items-center justify-between gap-4 rounded-full border border-primary/20 bg-primary px-4 text-left text-primary-foreground shadow-[var(--shadow-shell)] transition hover:-translate-y-0.5 md:bottom-8 md:left-auto md:right-8 md:w-auto md:min-w-[20rem] md:px-5"
    ...
    >
      <span className="flex min-w-0 items-center gap-3">
        ...
        <span className="min-w-0 pr-2">
          ...
          <span className="block text-xs text-primary-foreground/80">{itemCount} item{itemCount === 1 ? "" : "s"} - Rs {Number(cartTotal).toFixed(0)}</span>
        </span>
      </span>
