...
import { ArrowLeft, Banknote, Check, Clock, CreditCard, LoaderCircle, LocateFixed, MapIcon, MapPin, PackageCheck, Pencil, Search, ShieldCheck, UserRound, Wallet,
} from "lucide-react";
...
const QUICK_DESTINATIONS = ["Koramangala, Bengaluru", "HSR Layout, Bengaluru", "MG Road, Bengaluru",
] as const;
  ...
  return (
    <div className="relative h-[calc(100dvh-4rem)] min-h-[36rem] overflow-hidden bg-surface-muted">
      ...
      <aside className={cn(
        ...
        {stage === "route" ? (
          <CourierRoutePanel pickup={pickup} drop={drop} activePoint={activePoint} estimate={estimate.data} estimateLoading={estimate.isFetching} estimateError={estimate.isError} onActivePointChange={setActivePoint} onSelectLocation={beginPinning} onContinue={() => setStage("details")} />
        ) : null}
        {stage === "pin" && pinMode && pinDraft ? (
          <CourierPinPanel point={pinMode} location={pinDraft} addressBusy={pinAddressBusy} error={locationError} onBack={cancelPinning} onConfirm={confirmPinnedLocation} />
        ) : null}
        {stage === "details" ? (
          <CourierDetailsPanel pickup={pickup} drop={drop!} estimate={estimate.data} estimateLoading={estimate.isFetching} sender={sender} recipient={recipient} packageDescription={packageDescription} packageWeightKg={packageWeightKg} paymentMethod={paymentMethod} booking={createMutation.isPending} bookingError={createMutation.error} onBack={() => setStage("route")} onEditLocation={editLocation} onSenderChange={setSender} onRecipientChange={setRecipient} onPackageDescriptionChange={setPackageDescription} onPackageWeightChange={setPackageWeightKg} onPaymentChange={setPaymentMethod} onBook={() => createMutation.mutate()} />
        ) : null}
      </aside>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        ...
        <div className="mt-5 border-t border-border pt-4" aria-live="polite">
          ...
          {estimateLoading ? (
            ...
            <p className={cn("mt-2 text-sm", estimateError ? "text-destructive" : "text-muted-foreground",
              )}>{estimateError ? "Route pricing is unavailable. Try the address again." : "Add the delivery location to see distance, ETA, and fare."}</p>
          )}
        </div>
      </div>
...
function CourierLocationFields({ pickup, drop, activePoint, onEdit,
  ...
  return (
    <div className="relative rounded-lg border border-border bg-surface p-3 shadow-sm">
      ...
      <button type="button" className={cn("relative flex min-h-12 w-full items-center gap-3 rounded-md px-1 text-left hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30", activePoint === "pickup" && "bg-courier-soft/50",
      ...
      <button type="button" className={cn("relative flex min-h-12 w-full items-center gap-3 rounded-md px-1 text-left hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30", activePoint === "drop" && "bg-surface-muted",
        )} onClick={() => onEdit("drop")}>
        <MapPin className="z-10 size-4 shrink-0 fill-destructive text-destructive" aria-hidden="true" />
        <span className="min-w-0 flex-1"><span className="block text-xs font-medium text-muted-foreground">Deliver To</span><span className={cn("block truncate font-semibold", drop ? "text-sm text-foreground" : "text-base text-muted-foreground",
            )}>{drop?.address ?? "Search delivery address"}</span></span>
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex min-h-12 items-center gap-3 rounded-lg border border-border bg-surface px-3 focus-within:border-primary focus-within:ring-2 focus-within:ring-ring/20">
        {loading ? (
          <LoaderCircle className="size-4 shrink-0 animate-spin text-courier" aria-hidden="true" />
        ) : (
          <Search className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />
        )}
      ...
      {error ? (
        <p className="mt-2 rounded-md border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive" role="status">{error}</p>
      ) : null}
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-28 pt-4">
        ...
        {bookingError ? (
          <p className="rounded-md border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive" role="status">{bookingError.message || "The courier could not be requested. Check your connection and try again."}</p>
        ) : null}
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 border-t border-border bg-surface p-4 shadow-[0_-10px_24px_rgb(17,24,39,0.10)]">
        ...
        <p className="mt-2 text-center text-xs text-muted-foreground">
          Your final fare is confirmed before a delivery partner is assigned.</p>
      </div>
    </div>
  );
}
...
}: { title: string; value: CourierContactInput; onChange: (contact: CourierContactInput) => void;
}) {
  return (
    <div>
