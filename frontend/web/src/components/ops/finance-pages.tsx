...
export function OpsPayoutsPage() {
  ...
  const sweep = useMutation({ mutationFn: () => financePayoutSweep({ userRole: userRole || undefined, idempotencyKey: `ops:${userRole || "all"}:${Date.now()}`,
  ...
  return (
    <FinancePermissionBoundary action={PermissionAction.FinancePayoutManage}>
      <FinancePanel title="Payout Runs" icon={<Landmark className="size-5" aria-hidden="true" />} filters={<div className="grid gap-2 md:grid-cols-[12rem_auto]"><select className="rounded-md border border-border bg-surface px-3 text-sm" value={userRole} onChange={(e) => setUserRole(e.target.value)}><option value="">All partners</option><option>RESTAURANT</option><option>DELIVERY</option><option>DRIVER</option></select><Button onClick={() => sweep.mutate()} disabled={sweep.isPending}>Run sweep</Button></div>}>
        {sweep.data ? (
          <div className="rounded-md border border-border bg-surface p-3 text-sm text-muted-foreground">Created or queued {sweep.data.items.length} payouts{ sweep.data.duplicate ? " from an idempotent replay" : "" }.</div>
        ) : null}
        <QueryState isLoading={payouts.isLoading} isError={payouts.isError} error={payouts.error} onRetry={() => payouts.refetch()}>
          <FinanceTable headers={["Payout", "Partner", "Amount", "Bank", "State", "Action"]} empty="No payouts">
            {(payouts.data?.items ?? []).map((payout) => (
              <PayoutRow key={payout.id} payout={payout} onMark={() => mark.mutate(payout.id)} disabled={mark.isPending} />))}
          </FinanceTable>
        </QueryState>
      </FinancePanel>
    </FinancePermissionBoundary>
    ...
    <FinancePermissionBoundary action={PermissionAction.FinanceReconciliationRead}>
      <FinancePanel title="Razorpay Reconciliation" icon={<GitCompareArrows className="size-5" aria-hidden="true" />} filters={<div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]"><Input type="datetime-local" value={form.from} onChange={(e) => setForm((v) => ({ ...v, from: e.target.value }))} /><Input type="datetime-local" value={form.to} onChange={(e) => setForm((v) => ({ ...v, to: e.target.value }))} /><Button disabled={generate.isPending || !form.from || !form.to} onClick={() => generate.mutate()}>Run report</Button></div>}>
        <section className="rounded-md border border-border bg-surface p-4">
          ...
          {generate.error ? (
            <p className="mt-2 text-sm text-destructive">{generate.error instanceof Error ? generate.error.message : "Report failed"}</p>
          ) : null}
        </section>
        <QueryState isLoading={reports.isLoading} isError={reports.isError} error={reports.error} onRetry={() => reports.refetch()}>
          <FinanceTable headers={["Report", "Totals", "Matches", "State"]} empty="No reconciliation reports">
            {(reports.data?.items ?? []).map((report) => (
              <ReconciliationRow key={report.id} report={report} />))}
          </FinanceTable>
        </QueryState>
      </FinancePanel>
    </FinancePermissionBoundary>
          ...
          <QueryState isLoading={invoices.isLoading} isError={invoices.isError} error={invoices.error} onRetry={() => invoices.refetch()}>
            <FinanceTable headers={["Invoice", "Reference", "Customer", "Amount", "State"]} empty="No invoices">
              {(invoices.data?.items ?? []).map((invoice) => (
                <InvoiceRow key={invoice.id} invoice={invoice} onSelect={() => setSelected(invoice.id)} />))}
            </FinanceTable>
          </QueryState>
          <aside className="min-h-[32rem] rounded-md border border-border bg-surface p-3">
            {selected ? (
              <iframe title="Invoice document" src={financeInvoiceHtmlUrl(selected)} className="h-[30rem] w-full rounded-md border border-border bg-card" />
            ) : (
              <EmptyState title="No invoice selected" description="Generate or select an invoice to view it." />
            )}
          </aside>
        </div>
      </FinancePanel>
...
function FinancePermissionBoundary({ action, children,
  ...
  if (!access.can) {
    return (
      <EmptyState title="No access" description="This finance view is hidden by the permission matrix for your role." />
    );
  }
  return <>{children}</>;
}

function FinancePanel({ title, icon, filters, children,
}: { title: string; icon: ReactNode; filters?: ReactNode; children: ReactNode;
}) {
  return (
    <div className="space-y-4"><section className="rounded-md border border-border bg-surface p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="rounded-md border border-border bg-surface-muted p-2 text-brand">{icon}</span><h2 className="text-lg font-semibold text-foreground">{title}</h2></div><StatusPill label="Finance" tone="info" /></div>{filters ? <div className="mt-4">{filters}</div> : null}</section>{children}</div>
  );
}

function FinanceTable({ headers, empty, children,
}: { headers: string[]; empty: string; children: ReactNode;
}) {
  const rows = Array.isArray(children) ? children.filter(Boolean) : children;
  return (
    <div className="overflow-hidden rounded-md border border-border bg-surface"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-surface-muted text-xs uppercase text-muted-foreground"><tr>{headers.map((header) => (
...
function PayoutRow({ payout, onMark, disabled,
}: { payout: FinancePayout; onMark: () => void; disabled?: boolean;
}) {
  return (
...
function ReconciliationSummary({ report }: { report: FinanceReconciliationReport }) {
  return (
    <div className="mt-3 rounded-md border border-border bg-surface-muted p-3 text-sm"><p className="font-semibold text-foreground">Report {report.status}</p><p className="mt-1 text-muted-foreground">Provider Rs {Number(report.providerTotal).toFixed(2)} vs ledger Rs {" "}
        {Number(report.ledgerTotal).toFixed(2)}. Mismatches: {report.mismatchCount}.</p></div>
  );
}
...
function ReconciliationRow({ report }: { report: FinanceReconciliationReport }) {
  ...
  return (
    <tr className="border-t border-border"><Cell><Primary text={new Date(report.createdAt).toLocaleString()} sub={`${report.from} to ${report.to}`} /></Cell><Cell><Primary text={`Provider ${report.providerTotal}`} sub={`Ledger ${report.ledgerTotal}`} /></Cell><Cell>{report.matchedCount} matched / {report.mismatchCount} flagged{mismatches[0] ? (
          <p className="mt-1 max-w-sm truncate text-xs text-muted-foreground">{JSON.stringify(mismatches[0])}</p>
        ) : null}</Cell><Cell><StatusPill label={report.status} tone={report.status === "CLEAR" ? "success" : "danger"} /></Cell></tr>
  );
}
